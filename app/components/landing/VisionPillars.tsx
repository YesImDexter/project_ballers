...
export default function VisionPillars() {
  return (
    <section id="vision" className="scroll-mt-24 border-t border-light-border/50 bg-card/40 py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <Reveal group className="grid gap-5 md:grid-cols-2">
          {pillars.map((p) => (
            <article
              key={p.num}
              className="pillar reveal-item rounded-xl border border-light-border bg-card p-7 md:p-8"
            >
              <div className="mb-4 font-headings text-4xl font-bold italic text-accent">{p.num}</div>
              <h3 className="mb-2 font-headings text-2xl font-bold text-near-black">{p.title}</h3>
